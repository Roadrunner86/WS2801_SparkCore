...
void WS2801::updatePins(uint8_t dpin, uint8_t cpin) {
  ...
  //not defined in spark core to be added
...
void WS2801::startSPI(void) {
    ...
    SPI.setClockDivider(SPI_CLOCK_DIV32);  // 2 MHz
...
void WS2801::setPixelColorAll(uint32_t c){
	uint16_t i;
	for(i=0; i<numLEDs; i++){
		setPixelColor(i, c);
	}
...
void WS2801::setPixelColorAll(uint8_t r, uint8_t g, uint8_t b){
	uint16_t i;
	for(i=0; i<numLEDs; i++){
